...
| --- | --- | --- | --- | --- | --- | --- |
| document projet | horloge globale (PPQN), clips, timelines (marqueurs de tempo/métrique, durée), pistes, notes, instruments, presets et mixage | `ProjectDocument` historisé par `ProjectStore` | ouverture du projet | oui, dans `StoredProject` et la section `document` du nouveau `.pianola` | oui, par transaction métier | faible à moyenne |
| workspace projet | clip et instrument actifs, playhead, tick/pitch visibles, zoom, grille et snap tonal par clip | `ProjectWorkspaceState`, projeté dans `EditorRuntime` | durée de vie du projet | oui, atomiquement avec le document et dans une section portable distincte | non | moyenne |
