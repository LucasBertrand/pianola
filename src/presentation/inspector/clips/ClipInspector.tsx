...
import type { ReadonlyRenderSignal } from "../../../editor-core/model/render-signal";
import type { PlayheadPosition } from "../../../editor-core/model/playhead-position";
...
type HierarchyDialogDraft =
  ...
  | {
      ...
      readonly concatenatableClipCount: number;
    }
  | {
  ...
  const hierarchyProps: HierarchyListProps = {
    ...
    onEditGroup(group): void {
      setDialogDraft({
        ...
        concatenatableClipCount: getClipPlaybackOrder(group.children).filter(
          (clipId) => props.projectState.clipsById[clipId]?.bypassEnabled === false,
        ).length,
      });
    },
    ...
    <section className="clip-inspector-section">
      ...
      {dialogDraft === null ? null : dialogDraft.mode === "delete-group" ? (
        ...
        <ClipGroupEditorDialog
          ...
          canConcatenate={
            dialogDraft.concatenatableClipCount > 0
            && dialogDraft.name.trim().length > 0
          }
          onConcatenate={() => {
            if (props.onConcatenateGroup(
              dialogDraft.groupId,
              dialogDraft.name.trim(),
            ) !== null) {
              setDialogDraft(null);
            }
          }}
